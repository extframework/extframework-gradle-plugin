...
import com.kaolinmc.kiln.partition.GradlePartitionLoader
import com.kaolinmc.kiln.partition.GradlePartitionMetadata
import com.kaolinmc.kiln.source.DefaultSourcesManager
...
import com.kaolinmc.kiln.util.removePrefix
import com.kaolinmc.kiln.util.setupProject
...
open class DefaultExtensionInitializer(
    ...
    override val dataDir = root.layout.projectDirectory.asFile.toPath() resolve ".kaolin"
    ...
    override val managed: MutableSet<KaolinExtension> = HashSet()
    ...
    override suspend fun bootstrap(
        ...
        extension.build.parents += parents.map {
            KaolinExtension.BuildCache.ParentMetadata(
                it,
     ...
     *  - Sets up partitions
     */
    private val configured = HashSet<KaolinExtension>()
...
//        extension.environments += environments
    ...
    private suspend fun setupPartitions(
        ...
        val parentBuilds = config.parents.filter {
            ...
            val parentExtension = parentProject.extensions.getByType(KaolinExtension::class.java)
...
//            requireResourceVerification = true
    ...
    private abstract inner class Helper(
        ...
        private val accessibleParents: Map<ExtensionDescriptor, KaolinExtension>,
...
//                    requireResourceVerification = true
    ...
    private sealed interface DependencyType {
        fun toNotation(extension: KaolinExtension): Any
    }

    private data class SourceSetDependency(
        val sourceSet: SourceSet,
    ) : DependencyType {
        override fun toNotation(extension: KaolinExtension): Any {
